...
ResourceKey key1 = key("example.bundle", "someKey");

// bundle=example.MyService id=someKey
ResourceKey key2 = key(MyService.class, "someKey");

// bundle=example.MyEnum id=SOME_VALUE
ResourceKey key3 = key(MyEnum.SOME_VALUE);

// bundle=example.MyService id=null
ResourceKey bundle = bundle(MyService.class);
...
ResourceKey enumChild = key(MyEnum.VALUE, "child");
```

Resource values
---------------
Initially resource value is returned as `OptionalString` wrapper: using methods from fluent DSL you can convert it to `MandatoryString` (`notNull()` and `orDefault()` methods), to value of another type (method `as()`) or get the wrapped string. Here are some examples:
...
String string = value.asIs(); // can be null here
...
int withDefault = integer.orDefault(5); // autoboxing
...
long required = resources.get(key).required(Long.class);
...
```java
import static com.github.resource4j.objects.parsers.ResourceParsers.*;

ResourceObject document = resources.contentOf("document.xml", in(Locale.US));
OptionalString string = document.parsedTo(string("utf-8"));
```
...
public class ConfigParser extends AbstractValueParser<MyConfig> {
    ...
    protected MyConfig parse(ResourceObject object) throws IOException {
        return (MyConfig) xstream.fromXML(object.asStream());
    }
}
```
...
Oh, by the way, we already have XStream parser in [Parsers](Parsers.md) library!
...
Next: [Configuring resources](Configuration.md)
